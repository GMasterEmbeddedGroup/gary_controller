...
CallbackReturn PIDControllerWithFilter::on_configure(const rclcpp_lifecycle::State &previous_state) {
    ...
    if (!this->check_parameter("p_filter_coefficient", rclcpp::ParameterType::PARAMETER_DOUBLE)) {
        return CallbackReturn::ERROR;
    }
    ...
    if (!this->check_parameter("d_filter_coefficient", rclcpp::ParameterType::PARAMETER_DOUBLE)) {
        return CallbackReturn::ERROR;
    }
...
bool PIDControllerWithFilter::check_parameter(const std::string &name, rclcpp::ParameterType type) {
